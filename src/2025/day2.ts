...
import { INPUT, SAMPLE_INPUT } from "./input/input-day2";
...
function sharedSetup(input: string = INPUT): {lines: Line[]} {
    const lines = input.split(',').filter(l => !!l).map(l => l.split('-')).map(parts => ({start: +parts[0], end: +parts[1]} as Line));
    return {lines};
}

function addInvalidIdsPart1(lines: Line[]) {
    let sum = 0;
...
execExamplePart1(() => {
    ...
    return addInvalidIdsPart1(lines);
})
