...
Your puzzle answer was 57451709.
*/
...
class Range {
    ...
    isInTargetRange(value: number) {
        return this.targetRangeStart <= value && value < this.targetRangeEnd;
    }
    ...
    revert(value: number) {
        const tranformedValue = (value - this.targetRangeStart) + this.sourceRangeStart;
        return tranformedValue;
    }
}
...
soilToFert.previousMapper = seedsToSoil;
fertToWater.previousMapper = soilToFert;
waterToLight.previousMapper = fertToWater;
lightToTemp.previousMapper = waterToLight;
tempToHumidity.previousMapper = lightToTemp;
humidityToLocation.previousMapper = tempToHumidity;
...
execPart2(() => {
    ...
    const ranges = [...seedConfigs].map(seedConfig => {
        const seedStart = +(seedConfig?.groups?.['seedStart'] ?? '0');
        const seedLength = +(seedConfig?.groups?.['length'] ?? '0');
        return new Range(`${seedStart} ${seedStart} ${seedLength}`);
    })
    for(let i = 0; i < Number.MAX_SAFE_INTEGER; i++) {
        const revertedToSeed = humidityToLocation.revert(i);
        const range = ranges.find(r => r.isInRange(revertedToSeed));
        if(range) {
            return i;
        }
    }
